...
--// Library
local library, pointers = loadstring(game:HttpGet("https://raw.githubusercontent.com/colloper/Osiris/main/Library.lua"))()
    local window = library:New({name = os.date("Osiris Pro Fix | fixed by colloper :3 | %A, %B", os.time())..os.date(" %d", os.time())..GetSubPrefix(os.date(" %d", os.time()))..os.date(", %Y.", os.time()), size = Vector2.new(554, 629), Accent = Color3.fromRGB(255,22,22)})
    ...
    do
        ...
        do 
            ...
            --
            loadstring(game:HttpGet("https://raw.githubusercontent.com/colloper/Osiris/main/ESP.lua"))()
        end 
        ...
        if pointers["aim_viewer_bypass"]:get() then 
            ...
            if Target ~= nil and pointers["lock_enabled"]:get() and Target.Character and not getgenv().BehindWall and getgenv().InFOVRange then
                if pointers["resolver"]:get() and pointers["resolver_keybind"]:is_active() then 
                    local endpoint = Target.Character[HitPart].Position + (ResolverPrediction * pointers["lock_prediction"]:get()) 
                    Remote:FireServer("UpdateMousePos", endpoint) 
                else 
                    local endpoint = Target.Character[HitPart].Position + (Target.Character.HumanoidRootPart.Velocity *  pointers["lock_prediction"]:get()) 
                    Remote:FireServer("UpdateMousePos", endpoint) 
                end 
            end
        end 
    ...
    Old = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(self,...)
        if tostring(self.Name) == "MainEvent" then 
            ...
            if pointers["aim_viewer_bypass"]:get() == false and getnamecallmethod() == "FireServer" and args[1] == "UpdateMousePos" and Target ~= nil and pointers["lock_enabled"]:get() and Target.Character and not getgenv().BehindWall and getgenv().InFOVRange then
                if pointers["resolver"]:get() and pointers["resolver_keybind"]:is_active() then 
                    args[2] = Target.Character[HitPart].Position + (ResolverPrediction * pointers["lock_prediction"]:get()) 
                else 
                    args[2] = Target.Character[HitPart].Position + (Target.Character.HumanoidRootPart.Velocity *  pointers["lock_prediction"]:get())
                end 
